...
{
    if (gpMetric.size() == 0)
        scheme_error("No metric available!");
...
{
    if (gpSolver.size() == 0)
        scheme_error("No GeodSolver available!");
...
{
    if (gpCamera.size() == 0)
        scheme_error("No camera available!");
...
{
    if (gpProjector.size() == 0)
        scheme_error("No projector available!");

    assert(k < gpProjector.size());
    return gpProjector[k];
}

GvsRayGen* GvsParser ::getRayGen(unsigned int k)
{
    if (gpRayGen.size() == 0)
        scheme_error("Kein Strahlgenerator vorhanden!");

    assert(k < gpRayGen.size());
    return gpRayGen[k];
}

GvsLightSrcMgr* GvsParser ::getLightMgr(unsigned int k)
{
    if (gpLightMgr.size() == 0)
        scheme_error("Keine Lichtquellen-Manager vorhanden!");

    assert(k < gpLightMgr.size());
    return gpLightMgr[k];
}

GvsLightSrc* GvsParser ::getLight(unsigned int k)
{
    if (gpLight.size() == 0)
        scheme_error("Keine Lichtquelle vorhanden!");

    assert(k < gpLight.size());
    return gpLight[k];
}

GvsSceneObj* GvsParser ::getSceneObj(unsigned int k)
{
    if (gpSceneObj.size() == 0)
        scheme_error("Kein Szene-Objekt vorhanden!");

    assert(k < gpSceneObj.size());
    return gpSceneObj[k];
}

GvsStMotion* GvsParser ::getMotion(unsigned int k)
{
    if (gpMotion.size() == 0)
        scheme_error("Keine Bewegung vorhanden!");
...
void GvsParser::getDevice(GvsDevice* device, unsigned int k)
{
    if (k >= gpDevice.size()) {
        fprintf(stderr,
            "Error: GvsParser::getDevice(): device '%d' exceeds number of available devices '%d'! Exiting now.\n", k,
            static_cast<int>(gpDevice.size()));
        exit(-1);
    }
    if (device == nullptr) {
...
void GvsParser::printAll(FILE* fptr) const
{
    ...
    do {
        fprintf(fptr, "#%3i:   ID-Name:%14s    (Typ:%14s) -->%3i\n", i, (gpTypeIDptr->first).c_str(),
            GvsTypeName[gpTypeIDptr->second.gvsType].c_str(), gpTypeIDptr->second.vectorID);
        gpTypeIDptr++;
        i++;
...
void GvsParser ::initStandard(GvsDevice* device)
{
...
void GvsParser::read_scene(const char* name)
{
    ...
    scheme_define(
        &sc, sc.global_env, mk_symbol(&sc, "comp-octree-object"), mk_foreign_func(&sc, gvsP_compound_octree_obj));
    ...
    scheme_define(
        &sc, sc.global_env, mk_symbol(&sc, "solid-ellipsoid"), mk_foreign_func(&sc, gvsP_init_solidellipsoid));
    scheme_define(
        &sc, sc.global_env, mk_symbol(&sc, "solid-background"), mk_foreign_func(&sc, gvsP_init_solidbackground));
