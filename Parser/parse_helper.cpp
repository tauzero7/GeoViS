...
int searchCharArray(const char** stringArray, const int num, const char* searchItem)
{
    ...
    for (int n = 0; n < num; n++) {
        if (strcmp(stringArray[n], searchItem) == 0)
            return n;
    }
    return -1;
}
...
#endif
    if (args == sc->NIL)
        scheme_error("gvs-print: no arguments");
    if (!is_pair(args))
        scheme_error("gvs-print: less arguments");
...
pointer gvsP_set_parameter(scheme* sc, pointer args)
{
    ...
    if (args == sc->NIL)
        scheme_error("set-parameter: no arguments");
    if (!is_pair(args))
        scheme_error("set-parameter: less arguments");
...
pointer gvsP_get_parameter(scheme* sc, pointer args)
{
    ...
    if (args == sc->NIL)
        scheme_error("get-parameter: no argumente");
    if (!is_pair(args))
        scheme_error("get-parameter: less arguments");
...
{
    if (args == sc->NIL)
        scheme_error("getenv: no argumente");
    if (!is_pair(args))
        scheme_error("getenv: less arguments");
...
std::map<std::string, GvsTypeID>::iterator getIDptr(GvsParseScheme* gpSc, const std::string& objName,
    const std::string& suchName, const char* token, const GvsType gtype, int num)
{
...
{
    if (!is_number(s_x))
        scheme_error(msg + ": integer expected");
    *x = ivalue(s_x);
}
...
{
    if (!is_character(s_c))
        scheme_error(msg + ": character expected");
    tx = string_value(s_c);
...
{
    if (!is_string(s_s))
        scheme_error(msg + ": string expected");
    s = string_value(s_s);
}
...
{
    if (!is_vector(s_vec))
        scheme_error(msg + ": vector value expected");
    if ((*sc.vptr->vector_length)(s_vec) != dim)
        scheme_error(msg + ": point is of wrong dimension");
    ...
    for (int i = 0; (i < dim) & (i < 4); i++) {
        ...
        if (is_number(elem))
            ...
            scheme_error(msg + ": point has non-numeric element");
    }
}
...
void get_double_vec(pointer s_pnd, int dim, double p[], const std::string msg)
{
    if (!is_vector(s_pnd))
        scheme_error(msg + ": vector value expected");
    if ((*sc.vptr->vector_length)(s_pnd) != dim)
        scheme_error(msg + ": point is of wrong dimension");
