...
//       delParamList
//----------------------------------------------------------------------------
void GvsParseScheme ::delParamList(void)
{
    ...
    for (size_t i = 0; i < numParameter; i++) {
        ...
        if (gpType == gp_string_int) {
            delete[] reinterpret_cast<int*>(paramListe[i]->val);
        }
        else if (gpType == gp_string_double) {
            delete[] reinterpret_cast<double*>(paramListe[i]->val);
        }
        else if (gpType == gp_string_bool) {
            delete[] reinterpret_cast<bool*>(paramListe[i]->val);
        }
...
#ifdef _WIN32
        if (strcmpi(arg_name, name) == 0)
            return true;
#else
        if (strcasecmp(arg_name, name) == 0)
            return true;
#endif
        num++;
    } while (num < static_cast<int>(numParameter));

    return false;
}

void GvsParseScheme ::setAllowedName(const std::string& name, const GvsParseParamType pType, const int dim)
{
...
void GvsParseScheme ::testParamNames(const char* name)
{
    ...
    for (size_t i = 0; i < numParameter; i++) {
        char* pName = reinterpret_cast<char*>(paramListe[i]->name);
        bool validName = allowedName(pName);
#ifdef GVS_VERBOSE
        ...
        if (!validName) {
            msg = name;
            msg.append(": ");
            msg.append(reinterpret_cast<char*>(paramListe[i]->name));
            msg.append(" is no valid parameter name!");
            scheme_error(msg);
        }
    }
...
bool GvsParseScheme ::getParameter(const char* name, std::string& sval, int& num)
{
    ...
    if (num >= static_cast<int>(numParameter))
        return false;
...
bool GvsParseScheme ::getParameter(const char* name, char* sval)
{
...
//       getParameter  (int)
//----------------------------------------------------------------------------
bool GvsParseScheme ::getParameter(const char* name, int& ival)
{
    int num = 0;
    int* arg_val;
    std::string msg;
...
bool GvsParseScheme ::getParameter(const char* name, double& dval)
{
    ...
    if (testArgType(name, num)) {
        ...
        if ((getAllowedParamType(name) != paramType) && (paramType != gp_string_gvs)) {
            ...
            arg_val = static_cast<double*>(paramListe[num]->val);
            if ((paramListe[num]->dim) != 1)
                return false;
            dval = arg_val[0];
...
bool GvsParseScheme ::getParameter(const char* name, double* dval, int& num, bool fixpos)
{
    int dim, dimExpected = 0;
...
bool GvsParseScheme ::getParameter(const char* name, bool& bval)
{
    int num = 0;
    std::string msg;
...
bool GvsParseScheme ::getParameter(const char* name, m4d::Matrix<double, 2, 3>* mat)
{
    int num = 0;
...
bool GvsParseScheme ::getParameter(const char* name, m4d::Matrix<double, 3, 4>* mat)
{
    int num = 0;
...
{
    assert(k < static_cast<int>(numParameter));
...
int GvsParseScheme ::getParamDim(const int k) const
{
    assert(k < static_cast<int>(numParameter));
...
int GvsParseScheme ::getParamDim(const char* name) const
{
    ...
    do {
        i++;
    } while ((i < static_cast<int>(numParameter)) && (strcmp(name, (char*)paramListe[i]->name) != 0));
...
//
GvsParseParamType GvsParseScheme ::getParamType(const int k) const
{
    assert(k < static_cast<int>(numParameter));
    return paramListe[k]->type;
}

GvsParseParamType GvsParseScheme ::getParamType(const char* name) const
{
    int i = -1;
    do {
        i++;
...
//    gecastet.
//
pointer GvsParseScheme ::parse(pointer a)
{
#ifdef GVS_VERBOSE
...
void GvsParseScheme ::print() const
{
    ...
    printf("--------------------------------------\n");
    printf("Number of parameters: %d\n", static_cast<int>(numParameter));
    for (size_t i = 0; i < numParameter; i++) {
        int dim = paramListe[i]->dim;
        printf("Parameter %2d:  name: %13s   type: %8s   dim: %2d", static_cast<int>(i), (char*)paramListe[i]->name,
            GvsParseParamTypeName[paramListe[i]->type].c_str(), dim);
        if (dim >= 0) {
            int* ival;
            double* dval;
            bool bval;
