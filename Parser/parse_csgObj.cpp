...
pointer gvsP_init_csg_obj(scheme* sc, pointer args)
{
    ...
    if (args == sc->NIL)
        scheme_error("csg-obj: no arguments");
    if (!is_pair(args))
        scheme_error("csg-obj: less arguments");

    std::string allowedNames[4] = { "csgType", "obj1", "obj2", "id" };
    GvsParseAllowedNames allowedTypes[4] = {
        { gp_string_int, 1 }, // csgType
    ...
    if (csgType == CSG_Obj) {
        ...
        std::cerr << "Error: csgtype = " << csgType << ". Invalid CSG_ObjType. CSG_ObjType was set to gpCSGDifferObj."
                  << " Next time choose gpCSGDifferObj = 1, gpCSGUnifiedObj = 2, gpCSGIntersecObj = 3" << std::endl;
        exit(0);
    }
        ...
        //      csgObj = new GvsSolidDifferObj(child1,child2);
        std::cerr << "Error: csgtype = " << csgType << ". Invalid CSG_ObjType."
                  << " Next time choose gpCSGDifferObj = 1, gpCSGUnifiedObj = 2, gpCSGIntersecObj = 3" << std::endl;
        CSGTYPE = "gpCSGDifferObj";
