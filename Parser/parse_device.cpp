...
pointer gvsP_init_device(scheme* sc, pointer args)
{
...
#endif
    if (args == sc->NIL)
        scheme_error("init-device: no arguments");
    if (!is_pair(args))
        scheme_error("init-device: less arguments");
    ...
    do {
        ...
        if (setparamFound) {
            ...
            // fprintf(stderr,"Set change value: %s %s
...
#endif
    if (args == sc->NIL)
        scheme_error("set-changeobj: no arguments");
    if (!is_pair(args))
        scheme_error("set-changeobj: less arguments");

    std::string allowedNames[] = { "device", "setparam" };
    GvsParseAllowedNames allowedTypes[] = {
        { gp_string_string, 0 }, // device
        { gp_string_setparamlist, 0 } // setparam
    };
